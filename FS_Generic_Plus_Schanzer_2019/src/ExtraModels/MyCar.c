...
VehicleModel_New (struct tInfos *Inf)
{
    ...
    outMask = OT_MIN | OT_SUV | OT_TUV | OT_NUV;
    if ((Car.Road.RE = RoadNewRoadEval (Env.Road, RB_CRG, outMask, "MyCar")) == NULL) {
...
{;
    ...
    if (RoadRouteEval (Car.Road.RE, msgpre, IT_XY_S, &rIn, &rOut) != ROAD_Ok)
	goto ErrorReturn;
    if (!rOut.onRoad) {
