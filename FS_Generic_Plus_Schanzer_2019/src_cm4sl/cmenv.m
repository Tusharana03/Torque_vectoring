...
%
	...
	cminstdir = 'C:/IPG/hil/win32-7.0.3';
    else
	cminstdir = cmlocaldir	% for mat: CM-7.0
    end
...
%    end
	...
	try_addpath(fullfile(dirname(pwd), 'src_mcm4sl'));
...
% Issue an error message stating incompatibility with the running Matlab version.
    ...
    iscm4ds = ~isempty(strfind(cminstdir, '/win32-ds'));

    if iscm4ds && nbits==32
	msg = sprintf('\n%d-bit Matlab R%s is incompatible with any of the dSPACE versions\nsupported by this CarMaker release. See the CarMaker release notes\nand the dSPACE documentation for a list of supported versions.', nbits, version('-release'));
    else
